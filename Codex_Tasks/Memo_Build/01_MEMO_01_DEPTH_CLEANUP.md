# Memo Task — MEMO 01 Depth Cleanup
...
Clarify and fix depth semantics for the already completed first memo.
...
- `short` = графический executive pack, а не сжатая полная записка.
- `deep` = finance working package / Excel slices / evidence package, а не длинный дубль standard.
- `action` = tracker действий, а не narrative memo.

## Current concerns

- Проверить, не стал ли `short` просто урезанной текстовой запиской.
- Проверить, не стал ли `deep` дублем standard с appendix.
- Проверить, есть ли полноценный action tracker.
- Проверить активную структуру `06_reports/`.
- Не форсировать переименование папок без approval.

## Expected outputs

- Current-state review.
- Minimal correction plan.
- Если approval получен: исправленные README / inventory / file placement.
- QA summary.
